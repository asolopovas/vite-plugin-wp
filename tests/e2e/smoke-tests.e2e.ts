...
base.describe('Asset Loading @prod', () => {
    ...
    base('backend assets load correctly without dev mode', async ({ browser }) => {
        ...
        const cssHref = linkHrefs.find((h) => h?.includes('/static/build') && /-[a-zA-Z0-9_-]+\.css/.test(h))
    ...
    base('frontend assets load correctly without dev mode', async ({ browser }) => {
        ...
        const frontendCssHref = frontendLinkHrefs.find(
            (h) => h?.includes('/static/build') && /-[a-zA-Z0-9_-]+\.css/.test(h) && !h?.includes('editor')
        )
