...
const SOURCE_FILE = process.env.WP_TEST_BLOCK_SOURCE ?? path.join(PROJECT_ROOT, 'src/blocks/Test/index.tsx')
const CSS_SOURCE_FILE = process.env.WP_TEST_BLOCK_CSS ?? path.join(PROJECT_ROOT, 'src/styles/test.css')
...
test.describe('HMR @dev @hmr', () => {
    test.beforeAll(() => {
        ...
        if (!fs.existsSync(SOURCE_FILE)) {
            throw new Error(
                `HMR source fixture not found at ${SOURCE_FILE}. Set WP_PROJECT_ROOT or WP_TEST_BLOCK_SOURCE.`
            )
        }
        if (!fs.existsSync(CSS_SOURCE_FILE)) {
            throw new Error(
                `HMR css fixture not found at ${CSS_SOURCE_FILE}. Set WP_PROJECT_ROOT or WP_TEST_BLOCK_CSS.`
            )
        }
    })
    ...
    test('refreshes editor stylesheet with minimal requests', async ({ editorPage, wpHost }) => {
        ...
        const waitForColor = async (expected: string) => {
            ...
            const debug = await hmrPage.evaluate(() => {
                ...
                const hmrLinks = Array.from(doc?.querySelectorAll('link[data-wpv-hmr-path]') || []).map((link) =>
                    link.getAttribute('href')
                )
                const styleIds = Array.from(doc?.querySelectorAll('style[id]') || []).map((style) =>
            ...
            throw new Error(
                `HMR color did not update to ${expected}. Block: ${JSON.stringify(blockInfo)}. Debug: ${JSON.stringify(debug)}`
            )
        }
